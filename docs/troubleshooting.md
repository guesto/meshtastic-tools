...
3. Check current port permissions:
   ...
   ls -la /dev/ttyUSB0
   ```
   Expected output: `crw-rw---- 1 root dialout ...`

### Systemd service not starting

1. Check service status:
   ```
   systemctl status meshtastic-logger@DEVICE_NAME.service
   ```

2. Check timer status:
   ```
   systemctl status meshtastic-logger@DEVICE_NAME.timer
   ```
...
5. View systemd journal:
   ```
   journalctl -u meshtastic-logger@DEVICE_NAME.service -n 50
   ```
...
3. Create config from example:
   ...
   sudo cp /opt/meshtastic-tools/config/meshtastic-tools.yaml.example /etc/meshtastic-tools/config.yaml
   ```
...
3. Log files location (when using systemd service):
   ```
   cat /opt/meshtastic-tools/logs/DEVICE_NAME.log
   cat /opt/meshtastic-tools/logs/DEVICE_NAME-error.log
   ```
...
3. Check file permissions in data directory:
   ```
   ls -la /opt/meshtastic-tools/data/logger/
   ```

4. Verify storage directory exists:
   ```
   ls -la /opt/meshtastic-tools/data/logger/DEVICE_NAME/info/
   ```
